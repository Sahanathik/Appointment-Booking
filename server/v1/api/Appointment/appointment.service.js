...
async function appointment(req, res, next){
  ...
  let details = await appointmentSchema.findOne({patient_id:patientId}).exec();
  if(details){
  ...
  return res.json({status:'success', message:'appointment booked', result:result})
  } catch (error) {
 ...
 async function Booked(req, res, next){
    try {
        await appointmentSchema.find().then(data=>{
            return res.json({status:'success', message:'booked appointments', result:data})
        }).catch(err=>{
